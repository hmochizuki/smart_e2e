...
### Playwright ブラウザのインストール (録画機能を使う場合は必須)

Suite 編集画面の「録画から Step を作成」 (Playwright codegen) を使うには、ローカルに Playwright のブラウザバイナリが必要。

```sh
pnpm exec playwright install
```

未インストールの状態で録画を開始すると、ダイアログに「Playwright のブラウザがインストールされていません」と表示される。
